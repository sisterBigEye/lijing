...
def urlitems(fyurl):
    ...
    if m[0]==u'03ce3b5deb8544082161b5f8a5cd9436':##这个字符串根据具体数据库改变
        ...
        cur.execute("SELECT url FROM urltable  ORDER BY id ;")
        m1=(cur.fetchone())[0]
    print m1
...
def yinjianju_jx(url):
    ...
    for i in range(len(url)):
        ...
        if re.search(ur'个人姓名',tbody):
            try:
                result[name[0]]=driver.find_element_by_xpath('//*[@id="doc"]/center/div[3]/div[1]/div/div/table/tbody/tr[1]/td/table/tbody/tr[2]/td/div/div/table/tbody/tr[1]/td[2]').text.encode('asi')
            except:
                result[name[0]]=driver.find_element_by_xpath('//*[@id="doc"]/center/div[3]/div[1]/div/div/table/tbody/tr[1]/td/table/tbody/tr[2]/td/div/div/div/table/tbody/tr[1]/td[2]').text
...
def newadd_urldetail(fyurl,urls):
    ...
    for i in range(len(fyurl)):
        ...
        for news in soup.select('.STYLE8'):
            ...
            elif re.search(ur'处罚信息公开表',news['title']) and not re.search(ur'大连',news['title']):
            # if len(news.select('a'))>0:
                a = news['href']#获取href属性值
                # print a
                urldl.append(xingzhengchufaurl.format(a))

            elif re.search(ur'处罚信息公开表',news['title']) and not re.search(ur'吉林',news['title']):
            # if len(news.select('a'))>0:
                a = news['href']#获取href属性值
                # print a
                urljl.append(xingzhengchufaurl.format(a))

            elif re.search(ur'处罚信息公开表',news['title']) and not re.search(ur'江西',news['title']):
            # if len(news.select('a'))>0:
                a = news['href']#获取href属性值
                # print a
    ...
    print newall_d
    with codecs.open('newdaa.csv','a+','utf-8') as csvfile:
