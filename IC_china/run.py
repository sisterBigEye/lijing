...
#股东信息数据处理
def con_data_s(total):
    data=[]
    for i in range(1,len(total)):
    #range从1开始是因为股东信息的第一行是字段名称，这里不需要，真正的股东信息从第二行开始，也就是列表里的第二的开始。total=[{},{}]
        data.append(total[i].values())
    return data
#营业执照信息数据处理
#result=[{}]
def con_data_b(result):
    data=[]
    data.append(result[0].values())
    return data
#主要人员信息数据处理
def con_data_k(Con_info1):
    data=[]
