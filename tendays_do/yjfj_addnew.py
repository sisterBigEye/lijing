...
def urlitems(fyurl):
    ...
    try:
        cur.execute("INSERT INTO yjfj_url (urlhs) VALUES ('%s')"%urlshash[0])
    except:
        pass
    cur.close()
    conn.close()
...
def newadd_urldetail(urls):
    ...
    with codecs.open('yjfj%s.csv'%csvname,'a+','utf-8') as csvfile:
...
newadd_urldetail(urlitems(fenye(fenyeurl,5)))
driver.close()
